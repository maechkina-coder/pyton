...
# alias.field  or  alias."Quoted Field"  references inside expressions
# Group 1: alias  |  Group 2: quoted field (no quotes)  |  Group 3: unquoted field
ALIASED_FIELD = re.compile(
...
def validate_statement(stmt: str) -> list:
    """
    Perform structural validation on a SQL statement.
    Returns a list of error/warning strings; empty list means no issues found.

    Checks:
      1. Unclosed block comment  /* without */
      2. Unclosed string literal  ' or " without closing quote
      3. Unmatched / extra closing parentheses
      4. Unclosed parentheses at end of statement
      5. CASE without matching END
      6. END without preceding CASE
      7. No SELECT keyword found (after stripping comments)
      8. SELECT found but no FROM clause (warning — valid for SELECT 1+1 etc.)
    """
    issues    = []
    i         = 0
    n         = len(stmt)
    ...
    in_block_cmt = False
    in_line_cmt  = False
    case_stack   = 0          # net open CASE count (whole statement, all depths)
    ...
    while i < n:
        ...
        if in_block_cmt:
            ...
            if ch == str_ch:
                if i + 1 < n and stmt[i + 1] == str_ch:   # doubled-quote escape
                    i += 2
                    continue
                in_str = False
        elif two == '/*':
            ...
            if depth < 0:
                issues.append(
                    f"Syntax: extra closing parenthesis ')' at position {i}"
                )
                depth = 0
        else:
            # Keyword checks (only when not inside string/comment)
            b = stmt[i - 1] if i > 0 else ' '
            def _word_boundary(c):
                return not (c.isalnum() or c in '_$')

            if upper[i:i + 4] == 'CASE' and _word_boundary(b) and \
               _word_boundary(stmt[i + 4] if i + 4 < n else ' '):
                case_stack += 1

            elif upper[i:i + 3] == 'END' and _word_boundary(b) and \
                 _word_boundary(stmt[i + 3] if i + 3 < n else ' '):
                case_stack -= 1
                if case_stack < 0:
                    issues.append(
                        f"Syntax: END at position {i} has no matching CASE"
                    )
                    case_stack = 0
    ...
    if in_block_cmt:
        issues.append("Syntax: unclosed block comment (/* without */)")
    if in_str:
        issues.append(f"Syntax: unclosed string literal (opened with {str_ch!r})")
    if depth > 0:
        issues.append(
            f"Syntax: {depth} unclosed parenthes{'is' if depth == 1 else 'es'}"
        )
    if case_stack > 0:
        issues.append(
            f"Syntax: {case_stack} CASE expression(s) missing END"
        )

    # ── Semantic checks on comment-stripped text ──────────────────────────────
    clean = strip_comments(stmt)

    if not re.search(r'\bSELECT\b', clean, re.IGNORECASE):
        issues.append("Structure: no SELECT keyword found")
    else:
        # Check that the outermost SELECT has a matching FROM
        # (warning only — SELECT CURRENT_DATE etc. are valid without FROM)
        sel_clause = get_raw_select_clause(stmt)
        if sel_clause and not re.search(r'\bFROM\b', clean, re.IGNORECASE):
            issues.append(
                "Structure: SELECT found but no FROM clause "
                "(ok for constant expressions, check if intentional)"
            )
...
def _extract_paren_content_at(sql: str, open_pos: int) -> str:
    """
    Given the index of an opening '(' in sql, return the text between that
    paren and its matching closing paren (exclusive).  Handles nesting.
    Returns '' if open_pos doesn't point at '('.
    """
    ...
    while i < len(sql):
        ...
        i += 1
    return sql[open_pos + 1:]   # unclosed paren — return what we have
...
def extract_alias(expr: str) -> tuple:
    ...
    # alias."Quoted Name"  without AS — use the quoted part as the column name
    quoted = re.match(r'^(?:[a-zA-Z_$][\w$]*\.)"([^"]+)"$', expr.strip())
    if quoted:
        return quoted.group(1).upper(), expr
...
def source_tables(raw_expr: str, alias_map: dict,
    ...
    # aliased references: alias.field  or  alias."Quoted Field"
    for m in ALIASED_FIELD.finditer(clean):
        alias = m.group(1).upper()
        field = (m.group(2) or m.group(3) or '').upper()  # group 2=quoted, 3=unquoted

        # Prefer precise lookup in derived column map.
        # Only skip the alias_map fallback if the inner row actually has a source.
        if derived_col_map:
            ...
            if inner_row is not None:
                ...
                if t:
                    ...
                    continue  # inner row has a source → don't double-add from alias_map
                # inner row found but source is blank → fall through to alias_map
    ...
    if not found:
        ...
        # Extended bare-column fallback: search all derived view / CTE column maps.
        # Useful for passthrough SELECTs that use bare column names (no alias prefix),
        # e.g.  SELECT DISTINCT MDS_product, ... FROM (...) sub
        # Only apply when the expression is a plain identifier (no dots, no calls).
        if derived_col_map:
            bare = re.match(
                r'^[a-zA-Z_$][\w$]*$',
                strip_comments(raw_expr).strip(),
            )
            if bare:
                col_up = bare.group(0).upper()
                hits = []
                for view_map in derived_col_map.values():
                    row = view_map.get(col_up)
                    if row:
                        t = row.get('Source Table(s)', '')
                        if t and t not in hits:
                            hits.append(t)
                if len(hits) == 1:       # unambiguous → use it
                    return hits[0]
...
def parse_column(col_expr: str, alias_map: dict, short_desc: str = '',
    ...
    # Detect column renames:  orig_column AS new_name  (where names differ).
    # Treat as derived so the original column name is visible in the output.
    if not derived:
        _simple = re.match(
            r'^(?:[a-zA-Z_$][\w$]*\.)?([a-zA-Z_$][\w$]*)$', raw.strip(),
        )
        if _simple and _simple.group(1).upper() != output_col:
            derived = '*'
    ...
    if not derived and derived_col_map:
        # Match  alias.field  or  alias."Quoted Field"
        plain = (
            re.match(r'^([a-zA-Z_$][\w$]*)\.([a-zA-Z_$][\w$]*)$', raw.strip())
            or re.match(r'^([a-zA-Z_$][\w$]*)\."([^"]+)"$', raw.strip())
        )
        if plain:
            ...
            if inner_row and inner_row.get('Derived') == '*':
                ...
                # Inherit base fields from the inner row's logic rather than
                # using the outer alias.field name (which is just the alias).
                inner_fields = inner_row.get('Base Field(s)', '')
                return {
                    ...
                    "Derived":                     derived,
                    "Base Field(s)":               inner_fields or ', '.join(fields),
...
def build_cte_col_maps(sql: str) -> dict:
    """
    For every CTE defined in  WITH name AS (...) ,  parse that CTE's inner
    SELECT and build a column-level metadata map so that outer-query references
    like  cte_name.COL  can inherit the physical source table(s), derivation
    flag, and logic from inside the CTE body.

    CTEs are processed in definition order (earlier CTEs are already resolved
    when a later CTE is parsed), enabling multi-hop resolution:
        FINAL_Coff.X  →  CHRG_OFF.X  →  physical table

    Returns:
        { CTE_NAME_UPPER: { COL_NAME_UPPER: row_dict } }
    where row_dict has the same keys as parse_column() output.
    """
    clean  = strip_comments(sql)
    result = {}   # grows as each CTE is processed in order

    for m in CTE_DEF_PATTERN.finditer(clean):
        cte_name = (m.group(1) or m.group(2)).upper().strip('"')
        # m.end() is one past the opening '(' — so the '(' is at m.end()-1
        content = _extract_paren_content_at(clean, m.end() - 1)
        if not content:
            continue
        ...
        # ── Build local derived_col_map for this CTE ──────────────────────────
        # Start with all previously resolved CTEs (by their CTE name as key).
        local_dcm = dict(result)

        # Register any aliases used for earlier CTEs inside this body.
        # e.g.  FROM CHRG_OFF chrg_off  →  local_dcm['CHRG_OFF'] already set;
        #       also set local_dcm['CHRG_OFF'] for the alias 'chrg_off' → same map.
        _clean_body = strip_literals(strip_comments(content))
        for _m2 in TABLE_REF_PATTERN.finditer(_clean_body):
            _tbl   = _m2.group(1).upper().strip('"')
            _alias = (_m2.group(2) or '').upper().strip('"')
            if _tbl in result:                       # references an already-known CTE
                _key = _alias if (_alias and _alias not in CLAUSE_WORDS) else _tbl
                local_dcm.setdefault(_key, result[_tbl])

        # Also resolve inline views (non-CTE subqueries) within this CTE body.
        _inner_views = build_derived_col_maps(content)
        for k, v in _inner_views.items():
            local_dcm.setdefault(k, v)

        col_map = {}
        for raw_col in split_columns_raw(inner_raw):
            col_expr = norm(strip_comments(raw_col))
            row = parse_column(col_expr, inner_alias_map, derived_col_map=local_dcm)
            if row:
                col_map[row['Output Column Name'].upper()] = row
        if col_map:
            result[cte_name] = col_map
...
def process_statement(stmt: str, verbose: bool = False) -> list:
    ...
    # Parse each CTE's SELECT so outer references like cte.COL resolve to
    # the physical table inside the CTE body (not just a blank).
    # Inline-view entries take priority (setdefault), CTEs fill the gaps.
    cte_col_map = build_cte_col_maps(stmt)
    for cte_name, col_map in cte_col_map.items():
        derived_col_map.setdefault(cte_name, col_map)

    # Register aliases used for CTEs in the outer FROM/JOIN clauses.
    # e.g.  FROM base_member_ids AS T  →  T should resolve via BASE_MEMBER_IDS columns
    # CTEs are in skip_names so their alias never lands in alias_map; fix that here.
    known_ctes   = set(cte_col_map.keys())
    _clean_outer = strip_literals(strip_comments(stmt))
    for _m in TABLE_REF_PATTERN.finditer(_clean_outer):
        _tbl   = _m.group(1).upper().strip('"')
        _alias = (_m.group(2) or '').upper().strip('"')
        if _tbl in known_ctes:
            _key = _alias if (_alias and _alias not in CLAUSE_WORDS) else _tbl
            if _key not in derived_col_map and _tbl in cte_col_map:
                derived_col_map[_key] = cte_col_map[_tbl]

    if verbose:
        print(f"  [tables] {dict(list(alias_map.items())[:12])}")
        all_views = list(derived_col_map.keys())
        if all_views:
            print(f"  [inner views + CTEs] {all_views}")
...
def main():
    ...
    parser.add_argument(
        "--validate-only",
        action="store_true",
        help="Check SQL statements for structural errors and exit without writing CSV",
    )
    ...
    all_rows, n_select, n_skip, n_invalid = [], 0, 0, 0
    ...
    for raw in statements:
        ...
        if issues:
            ...
            if args.validate_only:
                continue
            print("  Attempting to parse anyway (results may be incomplete)")
    ...
    if args.validate_only:
        total = n_select + n_invalid
        print(f"\nSession-init statements skipped : {n_skip}")
        print(f"SELECT statements validated     : {total}")
        print(f"  Valid                         : {n_select}")
    ...
    if n_invalid:
        print(f"Statements with validation issues: {n_invalid} (parsed anyway)")
