...
def source_tables(raw_expr: str, alias_map: dict) -> str:
    ...
    # fallback: bare column with no alias prefix — use all in-scope physical tables.
    # Condition: multi-part name (schema.table) OR key == value (unaliased short name).
    # Fix: compare uppercase so mixed-case table names are not missed.
    if not found:
