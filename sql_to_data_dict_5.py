...
def source_tables(raw_expr: str, alias_map: dict) -> str:
    ...
    # Only resolve if there is exactly ONE physical table in scope — otherwise
    # we cannot tell which table owns the column and leave it blank (ambiguous).
    # Fix: compare v.upper() == k so mixed-case table names are not missed.
    if not found:
